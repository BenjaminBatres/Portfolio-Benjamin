...
export default function ContactSection() {
  ...
  const sendEmail = async () => {
    setLoading(true);
    const requiredFields: (keyof FieldValues)[] = ["name", "email", "message"];
  ...
  return (
    <section id="contact" className="py-20 px-5 sm:px-8 overflow-hidden">
      <Toaster />
      <div className="max-w-360 mx-auto grid lg:grid-cols-2 gap-10">
        <div className="md:py-5">
          <div className="mb-10">
            <motion.h3
              ...
              className="border border-Gray w-fit py-1 px-3 rounded-full text-lg uppercase  bg-black/50 mb-3"
            >
              Get in touch
            </motion.h3>
            <motion.h2
            ...
            >
              Let's work
              <span className="text-Blue font-bold"> together</span>
            </motion.h2>
            <RevealParaText
              text={
              ...
              className={"text-lg text-Secondary lg:max-w-lg leading-none"}
            />
          </div>
          <motion.div
          ...
          >
            <div className="flex items-center justify-center h-13 w-13 shadow-section-title-container bg-[#0d0d0d] rounded-full">
              <MdOutlineEmail className="text-3xl text-Blue" />
            </div>
            <div>
              <p className="text-sm font-medium text-secondary">Email Me</p>
              <Link
                href="mailto:me@benjaminbatres.dev"
                className="text-xl font-medium text-secondary hover:text-Blue duration-300"
              >
          ...
          >
            {socials.map((social, id) => (
              <Link
                ...
                key={id}
                target="_blank"
                className="flex items-center justify-center h-13 w-13 shadow-section-title-container bg-Blue rounded-full hover:scale-110 duration-300"
              >
                {social.icon}
              </Link>
            ))}
        ...
        >
          {inputFields.map((field, id) => (
            <div key={id} className=" flex flex-col gap-4">
              <label className="text-xl font-light">{field.label}</label>
              {field.type === "textarea" ? (
                <textarea
                  className="py-6 px-7 bg-Gray rounded-[10px] sm:text-lg"
                  name={field.id}
                  id={field.id}
                ...
                <div className="mb-6">
                  <input
                    ...
                    className="py-6 px-7 bg-Gray rounded-[10px] w-full sm:text-lg"
                  />
                </div>
              )}
            </div>
          ...
          <button
            onClick={sendEmail}
            className="py-4 w-full mt-8 rounded-full bg-Blue flex justify-center items-center gap-2 text-lg md:text-2xl  hover:bg-transparent duration-300 cursor-pointer"
          >
            {loading ? (
